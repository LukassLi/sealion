...
export class SplitChildren extends QiaoSuanPattern {
    public split: SplitChildAction;
    public unbracket: RemoveBracketAction;

    public subPattern: QiaoSuanPattern;

    public get name(): string {
        return '两数相乘—拆数（和）用运算律(SplitChildren)';
    }
    ...
    public match(steps: TransformActionBase[]): boolean {
        if (
            steps.length > 1 &&
            steps[0] instanceof SplitChildAction
        ) {
            this.split = steps[0] as SplitChildAction;
